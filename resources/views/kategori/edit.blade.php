...
<form action="{{ route('kategori.update', $category->id_category) }}" method="POST" class="space-y-4">
    @csrf
    @method('PUT')

    <!-- ID Kategori -->
    <div class="flex items-center space-x-4">
        <label for="id_category" class="w-32 text-gray-700 font-bold">ID (Auto Increment)</label>
        <input type="text" id="id_category" name="id_category" 
               value="{{ old('id_category', $category->id_category ?? '') }}" 
               class="flex-1 shadow appearance-none border rounded py-2 px-3 bg-gray-200 text-gray-700 focus:outline-none focus:shadow-outline" 
               disabled placeholder="Auto Increment">
    </div>
    ...
    <!-- Tombol -->
    <div class="tambah-item flex space-x-4 mt-4">
