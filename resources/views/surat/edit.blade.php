...
<form action="{{ route('surat.update', $surat->id_document) }}" method="POST" class="space-y-4" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="flex items-center space-x-4">
        ...
        <select id="kategori" name="cat_id"  
            ...
            @foreach($cat as $kategori)
                <option value="{{ $kategori->id_category }}" 
                    {{ old('cat_id', $surat->cat_id ?? '') == $kategori->id_category ? 'selected' : '' }}>
    ...
    <div class="flex items-center space-x-4">
        ...
        <label id="file_label" for="file_pdf" 
            class="flex-1 shadow appearance-none border rounded py-2 px-3 text-gray-700 cursor-pointer focus:outline-none focus:shadow-outline">
            {{ $surat->file_path ?? 'Klik disini untuk mencari file' }}
        </label>
    ...
    @if (!empty($surat->file_path))
        <div class="ml-36 text-sm text-gray-600">
            File lama: 
            <a href="{{ asset('uploads/surat/' . $surat->file_path) }}" target="_blank" class="text-blue-600 underline">
                {{ $surat->file_path }}
            </a>
        </div>
    @endif
